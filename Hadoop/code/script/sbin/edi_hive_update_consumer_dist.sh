...
echo "1.clear table EDI_M_CONSUMER_DIST & insert new data."
hive -S -e "INSERT OVERWRITE TABLE EDI.EDI_M_CONSUMER_DIST
SELECT A.BRAND,A.MODEL,REGEXP_REPLACE(A.AREA,'^$','其他'),A.AMOUNT FROM (
SELECT I.BRAND,I.MODEL,C.USERPROVINCE as AREA,COUNT(C.ID) as AMOUNT FROM EDI.EDI_N_PROD_COMMS C 
...
table="M_CONSUMER_DIST"
file="/user/hive/warehouse/edi.db/edi_`echo $table |tr A-Z a-z`/"
echo "INFO:export file:$file"
sqoop export --connect $mysql_url --table $table --username edi --password edi@zy11 --export-dir $file --input-fields-terminated-by '\t' --input-null-string NULL --input-null-non-string NULL 
ecode=$?
