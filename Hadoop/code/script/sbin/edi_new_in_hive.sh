...
if [ $? -ne 0 ];then
	...
	ssh hadoop@HadoopMySQL mv "$fromdir/Comm" "$fromdir/comments_bk/Comm-$start_time"
	if [ $? -ne 0 ];then
...
if [ $? -ne 0 ];then
	...
	ssh hadoop@HadoopMySQL mv "$fromdir/Pro" "$fromdir/products_bk/Pro-$start_time"
	if [ $? -ne 0 ];then
        	echo "WARNING:HadoopMySQL:remote part-r-00000 rename failed."
	fi

	echo "7.INFO:local file to hive."
	sed -ie 's/^\t//' $newdir/product/part-r-00000
