...
--####LOAD DATA LOCAL INPATH '/opt/running/edi/data/new/product/part-r-00000' INTO TABLE EDI_M_PROD_INFO PARTITION (PT_DATE='20151226172010');
...
--###LOAD DATA LOCAL INPATH '/opt/running/edi/data/new/comment/part-r-00001' INTO TABLE EDI_M_PROD_COMMS PARTITION (PT_DATE='20151226102010');
...
CREATE TABLE IF NOT EXISTS EDI_R_COMM_TAG (
 ...
 ASPT_START STRING,
 ASPT_END   STRING,
 SENTI_START STRING COMMENT '情感词位置',
 SENTI_END   STRING
 )
...
--6.EDI_R_COMM_MENTION
CREATE TABLE IF NOT EXISTS EDI_R_COMM_MENTION (
...
-- 7.MODEL_MENTION
INSERT into TABLE EDI_M_MODEL_MENTION PARTITION(PT_DATE='20151217142010')
...
select mm.brand_s as mbs,mm.model_s as mms,ma.brand_s as abs,ma.model_s as ams,mm.amt as amt from (
  ...
  FROM (
    ...
    LEFT JOIN EDI_M_PROD_INFO i on i.PROD_ID=c.PROD_ID
...
CREATE TABLE EDI_M_MODEL_MENTION (
  BRAND_S STRING COMMENT '标准品牌',
  MODEL_S STRING COMMENT '标准型号',
  BRAND_M STRING COMMENT '提及品牌',
  MODEL_M STRING COMMENT '提及型号',
  AMOUNT INT COMMENT '提及次数'
) PARTITIONED BY (PT_DATE STRING)
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
STORED AS TEXTFILE;
...
-- 8.MODEL_ALIAS
LOAD DATA LOCAL INPATH '/opt/running/edi/mention/model_alias.txt' INTO TABLE EDI_M_MODEL_ALIAS;
CREATE TABLE EDI_M_MODEL_ALIAS (
  BRAND_S STRING COMMENT '标准品牌',
  MODEL_S STRING COMMENT '标准型号',
  ALIAS STRING COMMENT '型号别名',
  IS_BRAND INT
...
--INSERT OVERWRITE TABLE EDI_M_BRAND_MODEL SELECT BRAND,MODEL,count(MODEL) FROM EDI_M_PROD_INFO GROUP BY BRAND,MODEL;
--9.EDI_M_BRAND_MODEL
CREATE TABLE IF NOT EXISTS EDI_M_BRAND_MODEL (
 BRAND STRING COMMENT '品牌',
 MODEL STRING COMMENT '型号',
 AMOUNT INT COMMENT '数量'
 ) COMMENT '标准型号'
...
insert overwrite table edi_m_prod_params select transform(concat(A.prod_id,'\t',A.params)) USING 'prod_params_prase.py1' as a,b,c from (select * from edi_m_prod_info where pt_date=20151222172010) A;
--select transform(concat(A.prod_id,'\t',A.params)) USING 'prod_params_prase.py1' as a,b,c from (select * from edi_m_prod_info limit 3) A;
CREATE TABLE IF NOT EXISTS EDI_M_PROD_PARAMS (
 PROD_ID STRING COMMENT '商品ID',
 PARAM STRING COMMENT '规格参数',
 VAL STRING COMMENT '参数值'
 ) COMMENT '商品规格'
PARTITIONED BY (PT_DATE STRING)
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
STORED AS TEXTFILE;